...
from nets.third_party.llava.mm_utils import get_model_name_from_path
class ReviseVideoDataset(Dataset):
    """
    Dataset for loading pre-encoded features from pickle files for Revise model training.
...
def revise_collate_fn(batch):
    """
    Custom collate function for VideoDataset.
...
def create_revise_dataloader(file_path,args, batch_size=1, shuffle=True, num_workers=4, distributed=False, rank=0, world_size=1):
    """    
    Args:
    ...
    Returns:
        DataLoader: PyTorch DataLoader for the ReviseVideoDataset
    """
    dataset = ReviseVideoDataset(file_path,args)
...
def create_dummy_dataset(output_dir, num_samples=128, frames=21, height=32, width=32, emb_dim=4096, hidden_dim=1024):
    """
    Creates a dummy dataset of pickle files for testing the ReviseVideoDataset.
...
def test_revise_dataloader(output_dir=None, batch_size=2):
    """
    Test function for the ReviseVideoDataset and dataloader.
    ...
    try:
        ...
        # Create dataloader
        dataloader = create_revise_dataloader(txt_file_path, batch_size=batch_size, num_workers=0)
...
if __name__ == "__main__":
    ...
    os.makedirs(output_dir, exist_ok=True)
    test_revise_dataloader(output_dir)