...
from nets.revise.modules.adapter import DM_Adapter
from nets.revise.modules.visual_context_adapter import VisualContextAdapter
from nets.third_party.llava.model.vila_with_vision_head import VisionHead

class ReviseVideoMixedConditionModel(nn.Module):
    """    
    ...
    def __init__(
        ...
        if isinstance(wan_model_or_ckpt_dir, str):
            ...
            # if wan_checkpoint is not None:
            #     self.wan_model = WanModel.from_pretrained(wan_checkpoint)#wan_model_or_ckpt_dir
            # else:
            self.wan_model = WanModel.from_pretrained(wan_model_or_ckpt_dir)#wan_model_or_ckpt_dir
        else:
    ...
    def nan_hook(self,module, input, output):
        if torch.isnan(output).any() or torch.isinf(output).any():
            print(f"[Step {module.__class__.__name__}] NaN detected!")
        ...
        if condition_mode == "full":
                ...
                if isinstance(ar_vision_input, list): # do padding for aligned_emb to the longest length
                    ...
                    for idx in range(batch_size):
                        if ar_vision_input[idx] is not None:
                            ...
                            self.ar_vision_head.register_forward_hook(self.nan_hook)
                            if torch.isnan(ar_vision_head_output[idx]).any() or torch.isinf(ar_vision_head_output[idx]).any():
                                print(f"[ar_vision_head_output {idx}] !!! Loss is NaN/Inf !!!")
                            if x[idx].size(1) == 1:
                                ...
                                self.adapter.register_forward_hook(self.nan_hook)
                                if torch.isnan(adapter_output[idx]).any() or torch.isinf(adapter_output[idx]).any():
                                    print(f"[adapter_output {idx}] !!! Loss is NaN/Inf !!!")
                        else:
                            adapter_output[idx] = None
                else:
                    ar_vision_head_output = self.ar_vision_head(ar_vision_input) # [bs, 4, 1152] or [bs, 1, 1152],([1, 4, 1152])
                    if x[0].size(1) == 1:
                        ...
                        adapter_output = einops.rearrange(adapter_output, '(b n) c d -> b (n c) d', b=ar_vision_head_output.size(0)) #  [bs*4, 256, 4096] -> [bs, 4*256, 4096],([1, 1024, 4096])
            ...
            if visual_emb is not None and self.visual_context_adapter is not None:
                if isinstance(visual_emb, list):
                    ...
                    processed_visual_emb = self.visual_context_adapter(visual_emb)#([16, 21, 60, 104])->([1, 8190, 4096])
            ...
            for idx in range(batch_size):
                ...
                if adapter_output is not None:
                    if isinstance(adapter_output, list):
                        ...
                        adapter_item = adapter_output[idx]#([1, 1024, 4096])->([1024, 4096])
                    else:
                ...
                if context is not None:
                    if isinstance(context, list):
                        if context[idx].dim() == 3:
                            ...
                            context_item = context[idx]#([157, 4096])
                    elif context.dim() > 2:
                ...
                if processed_visual_emb is not None:
                    visual_item = processed_visual_emb[idx]#([1, 8190, 4096])->([8190, 4096])
                ...
                if special_token_dict is not None:
                    ...
                    if visual_item is not None and '<img_st>' in special_token_dict and '<img_ed>' in special_token_dict:
                        components.extend([
                            special_token_dict['<img_st>'],#([6, 4096])
                            visual_item,#([8190, 4096])
                            special_token_dict['<img_ed>']#([6, 4096])
                        ])
                    ...
                    if context_item is not None:
                        if '<prp_st>' in special_token_dict and '<prp_ed>' in special_token_dict:
                            components.extend([
                                special_token_dict['<prp_st>'],#([7, 4096])
                                context_item,#([157, 4096])
                ...
                mixed_context.append(new_context)#([1159, 4096])
        
        else:
            raise ValueError(f"Condition mode {condition_mode} is not supported")
        
        # Forward through WanModel with the enhanced context
        return self.wan_model(x, t=t, context=mixed_context, seq_len=seq_len)#([1065, 4096])
    
    def save_pretrained(self, save_dir: str):
        """
        Save the ReviseVideoMixedConditionModel to the specified directory.
