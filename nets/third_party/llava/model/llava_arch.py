...
from nets.third_party.llava import modals
...
class LlavaMetaModel(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_vision_tower()(images)#([8, 729, 1152])
        image_features = self.get_mm_projector()(image_features)#([8, 196, 4096])
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        image_features = self.encode_images(images).to(self.device)#([8, 3, 384, 384])
        ...
        input_embeds = self.llm.model.embed_tokens(input_ids_copy)#([1, 196, 4096]),([1, 28, 4096])
