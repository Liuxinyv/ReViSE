...
class VisionHead(Module):
    ...
    def forward(self, vlm_last_hidden_state):
        ...
        input_embeds = self.fc(vlm_last_hidden_state)#([1, 69, 4096])->([1, 69, 1152])
        vision_tokens = self.visionHeadAdapter(src=input_embeds, tgt=self.decoder_query.repeat(input_embeds.shape[0], 1, 1)) # B * 4 * 1152,([1, 4, 1152])
