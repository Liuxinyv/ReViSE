...
def read_video_frames(video_path, frame_num, sampling_rate=3, skip_num=0, target_size=(480, 832)):
    ...
    frames = transform_frames_to_tensor(frames, target_size)
    ...
    with open(args.video_list_path, 'r') as f:
        ...
        for idx, item in enumerate(video_list):
            try:
                ...
                gt_video_path = item['target_video_path']
                prompt = item['instruction']
                prompt = prompt.replace('\n', ' ')

                if not os.path.exists(video_path) or not os.path.exists(gt_video_path):
                    logging.warning(f"Video file does not exist, skipping: {video_path}")
                    continue
                
                video_name = os.path.splitext(os.path.basename(gt_video_path))[0]
                ...
                gt_frames_tensor = read_video_frames(gt_video_path, args.frame_num, sampling_rate, args.skip_num, target_size)
                if frames_tensor is None or gt_frames_tensor is None:
                ...
                with torch.no_grad():
                    ...
                    gt_frames_tensor = gt_frames_tensor.to(device)
                    latent_feature = vae.encode(frames_tensor.transpose(0,1).unsqueeze(0))
                    latent_feature_tgt = vae.encode(gt_frames_tensor.transpose(0,1).unsqueeze(0))
                    latent_feature = latent_feature[0].cpu()
                    latent_feature_tgt = latent_feature_tgt[0].cpu()
                ...
                data = {
                    ...
                    'latent_feature_tgt':latent_feature_tgt
                }
